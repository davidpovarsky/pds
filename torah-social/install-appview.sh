...
# ATPROTO_BRANCH / SOCIAL_APP_BRANCH: default to runtime-repair; override via env
ATPROTO_BRANCH="${ATPROTO_BRANCH:-codex/torah-social-runtime-repair}"
...
ATPROTO_BRANCH=${ATPROTO_BRANCH}
ATPROTO_SHA=$(git -C "${APPVIEW_SRC_DIR}" rev-parse HEAD)
SOCIAL_APP_BRANCH=${SOCIAL_APP_BRANCH}
SOCIAL_APP_SHA=$(git -C "${SOCIAL_APP_DIR}" rev-parse HEAD)
EOF
...
log "Verifying AppView/PDS topology consistency"

# 1. Verify the PDS pds.env now points at our local AppView
CONFIGURED_APPVIEW_URL="$(sed -n 's/^PDS_BSKY_APP_VIEW_URL=//p' "${STATE_DIR}/pds.env" | head -n 1)"
CONFIGURED_APPVIEW_DID="$(sed -n 's/^PDS_BSKY_APP_VIEW_DID=//p' "${STATE_DIR}/pds.env" | head -n 1)"
[[ "${CONFIGURED_APPVIEW_URL}" == "https://${APPVIEW_HOSTNAME}" ]] || \
  fail "PDS_BSKY_APP_VIEW_URL mismatch: got '${CONFIGURED_APPVIEW_URL}', expected 'https://${APPVIEW_HOSTNAME}'"
[[ "${CONFIGURED_APPVIEW_DID}" == "${APPVIEW_DID}" ]] || \
  fail "PDS_BSKY_APP_VIEW_DID mismatch: got '${CONFIGURED_APPVIEW_DID}', expected '${APPVIEW_DID}'"

# 2. Verify PDS_CRAWLERS is empty (no public relay)
CONFIGURED_CRAWLERS="$(sed -n 's/^PDS_CRAWLERS=//p' "${STATE_DIR}/pds.env" | head -n 1)"
[[ -z "${CONFIGURED_CRAWLERS}" ]] || \
  fail "PDS_CRAWLERS is not empty: '${CONFIGURED_CRAWLERS}' — public relay crawling must be disabled"

# 3. Semantic health check: AppView must respond with a valid JSON body
APPVIEW_HEALTH_BODY="$(curl --fail --silent --max-time 5 "https://${APPVIEW_HOSTNAME}/xrpc/_health")"
echo "${APPVIEW_HEALTH_BODY}" | grep -q '"version"' || \
  fail "AppView /xrpc/_health did not return expected JSON body: ${APPVIEW_HEALTH_BODY}"

log "Topology verification passed"
...
Public Bluesky crawler: DISABLED (PDS_CRAWLERS=)
...
atproto branch: ${ATPROTO_BRANCH}
social-app branch: ${SOCIAL_APP_BRANCH}

Topology verified: PDS→AppView DID and URL consistent.
