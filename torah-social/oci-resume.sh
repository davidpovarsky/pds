...
INSTANCE_ID="$("${OCI[@]}" compute instance list \
  ...
  --raw-output)"
[[ -n "${INSTANCE_ID}" && "${INSTANCE_ID}" != "null" && "${INSTANCE_ID}" != "None" ]] || fail "No existing ${INSTANCE_NAME} VM found."
echo "Instance: ${INSTANCE_ID}"

echo "==> Waiting for its VNIC and public IPv4"
PUBLIC_IP=""
...
[[ -n "${PUBLIC_IP}" && "${PUBLIC_IP}" != "null" && "${PUBLIC_IP}" != "None" ]] || fail "VM exists but Oracle still has not exposed a public IPv4 address."
...
ssh -o StrictHostKeyChecking=accept-new -i "${KEY_PATH}" "ubuntu@${PUBLIC_IP}" \
  'sudo tail -n 80 /var/log/torah-social-bootstrap.log 2>/dev/null || echo "Installer log not created yet."'
...
if ssh -o StrictHostKeyChecking=accept-new -i "${KEY_PATH}" "ubuntu@${PUBLIC_IP}" \
  ...
  echo "Torah Social is still installing. Run this same resume command again in a few minutes."
fi
